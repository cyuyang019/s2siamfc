...
class ExperimentVID_GOT(object):
    ...
    """
    def __init__(self, root_dir, list_file=None,
                 result_dir='results', report_dir='reports'):
        super(ExperimentVID_GOT, self).__init__()
        self.dataset = VIDVOT(root_dir, list_file=list_file)
        self.result_dir = os.path.join(result_dir, 'VID_valgot')
        self.report_dir = os.path.join(report_dir, 'VID_valgot')
        self.nbins_iou = 101
        self.repetitions = 3

    def run(self, tracker, visualize=False):
        print('Running tracker %s on VID...' % tracker.name)
...
#        covers = {s: self.dataset[s][2]['cover'][1:] for s in seq_names}
        ...
        for name in tracker_names:
            ...
            for s, (_, anno, meta) in enumerate(self.dataset):
                ...
                ao, sr, sr75, speed, _ = self._evaluate(seq_ious, seq_times)
            ...
            ao, sr, sr75, speed, succ_curve = self._evaluate(ious, times)
    ...
    def _evaluate(self, ious, times):
        ...
        return ao, sr, sr75, speed_fps, succ_curve
