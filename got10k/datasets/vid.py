...
#     """
#     def __init__(self, root_dir, subset=('train', 'val'),
#                  cache_dir='cache/imagenet_vid', cluster_dir='cluster_dict.json'):
#         self.root_dir = root_dir
#         self.cache_dir = cache_dir
...
#             seq_name = self.seq_names[index]
#         
#         seq_dir, frames, anno_file, clusterid = self.seq_dict[seq_name]
#         img_files = [os.path.join(
#             seq_dir, '%06d.JPEG' % f) for f in frames]
#         anno = np.loadtxt(anno_file, delimiter=',')
# 
#         return img_files, anno, clusterid
# 
#     def __len__(self):
#         return len(self.seq_dict)
# 
...
#             return seq_dict
#         
#         with open(self.cluster_dir) as f:
#             cluster_dict = json.load(f, object_pairs_hook=OrderedDict)       
#         
#         def get_key(val): 
...
#                 np.savetxt(cache_anno_file, anno, fmt='%d', delimiter=',')
#                 
# #                print(seq_dirs[s].rsplit('\\', 1)[-1])
#                 
#                 # store paths
# #                print(get_key(seq_dirs[s].rsplit('\\', 1)[-1]))
# #                print(seq_dirs[s].rsplit('\\', 1)[-1])
#                 
#                 try:
#                     cluster = int(get_key(seq_dirs[s].rsplit('\\', 1)[-1]))
#                     assert isinstance(cluster, int)
#                 except:
#                     cluster = np.random.randint(10)
#                     print(seq_dirs[s].rsplit('\\', 1)[-1])
# 
#                 seq_dict.update([(key, [seq_dirs[s], frames, cache_anno_file, cluster])])
#                 
# # =============================================================================
...
# =============================================================================
# class ImageNetVID_unsupervised(object):
#     r"""`ImageNet Video Image Detection (VID) <https://image-net.org/challenges/LSVRC/2015/#vid>`_ Dataset.
# 
#     Publication:
#         ``ImageNet Large Scale Visual Recognition Challenge``, O. Russakovsky,
...
#     """
#     def __init__(self, root_dir, subset=('train', 'val'),
#                  cache_dir='cache/imagenet_vid_unsupervised'):
#         self.root_dir = root_dir
#         self.cache_dir = cache_dir
#         if isinstance(subset, str):
#             assert subset in ['train', 'val']
...
# #             anno_files = sorted(glob.glob(os.path.join(
# #                 anno_dirs[s], '*.xml')))
# #             anno_file_first = anno_files[0]
# #             object_first = ET.ElementTree(file=anno_file_first).findall('object')
# #             
# #             # find all track ids
# #             track_ids, counts = np.unique([
# #                 obj.find('trackid').text for obj in object_first], return_counts=True)
# #             
# #             
# #             # fetch paths and annotations for each track id
# #             for t, track_id in enumerate(track_ids):
