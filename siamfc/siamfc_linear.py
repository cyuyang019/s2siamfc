...
#            score_map = cv2.normalize(responses_ori[scale_id], None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
            for sid in range(self.cfg.scale_num):
                ...
                cv2.imwrite("./test/demo_score_map_%d/score_map_%d.jpg"%(sid,fid), np.uint8(25 * score_map))
                torchvision.utils.save_image(x_tensor[sid], './test/demo_search_tensor_%d/%d.jpg'%(sid,fid))
